...
### Fixed
- **Source parser performance: single-pass tokenization** — `withSource`/`__source__` source parsing now uses a single tokenization pass that caches all declarations, instead of re-tokenizing the entire file for every function/doc lookup. This dramatically reduces compile times for large source files (reported 2m21s → seconds).
